...
# Code based on: https://blog.grimm-co.com/post/guided-fuzzing-with-driller/
...
def save_input(content, dest_dir, count):
    ...
    print 'Saving drilled content '
    print map(lambda c: hex(ord(c)), content)
...
def main():
    ...
    while True:
		...
		# Truncate the testcase to 200 bytes (large testcases causes full CPU usage)
		if len(seed) > 200:
			print 'Truncate seed'
			seed = seed[:200] 
